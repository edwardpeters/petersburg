...
use crate::geography::grid::StepResult;
use crate::geography::maze::Maze;
use cairo::Context;
use general::burg::Burg;
use general::constants::*;
use general::direction::*;
use general::draw_utils::*;
use general::grid::{Grid, Point, RwGrid};
...
impl Foodburg {
            ...
            match actors.pop() {
                ...
                Some(SporeSpawn {
                    ...
                    *active_count += 1;
                    ...
                    let result = self.attempt_grow_pathed(spawn_p, s);
    ...
    fn attempt_grow_pathed(&self, spawn_point: Point, s: SpeciesID) -> GrowResult {
        ...
        use general::grid::{Neighborhood, PathResult, StepResult};
    ...
    fn grow_food(&self) {
        ...
        'ripen: loop {
            ...
            let mold_count = ring.count_matching(|s| match s {
                Square::Mold { .. } => true,
                _ => false,
            });
            if mold_count > 0 {
        ...
        'seek: loop {
            ...
            let ring = self.grid.get_ring(p);
            let n_count = ring.count_matching(|s| s == Square::Wall || s == Square::Food);
            if n_count >= 4 {
    ...
    fn rand_descendent_leaf(&self, p: Point, s: SpeciesID) -> Option<Point> {
        println!("I knew this was not to be trusted");
        use rand::seq::SliceRandom;
